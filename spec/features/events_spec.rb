...
feature 'viewing a listing of events' do
  background do
    ...
    Event.create(name: 'Event 2', archived: false)
    Event.create(name: 'Event 3', archived: false)
  end

  scenario 'as an administrator' do
    sign_in with: {administrator: true}, via: :calnet

    expect(find('.event_list .event:nth-child(1) .name')).to have_text('Event 3')
    expect(find('.event_list .event:nth-child(2) .name')).to have_text('Event 2')
    expect(find('.event_list .event:nth-child(3) .name')).to have_text('Event 1')
  end
