...
describe Visitor do
  ...
  describe '.with_contact_info' do
    ...
    before do
      Visitor.create!(event: event, user: User.create(first_name: 'John', last_name: 'Douglass'))
      Visitor.create!(event: event, user: User.create(first_name: 'Andy', last_name: 'Frank'))
      Visitor.create!(event: event, user: User.create(first_name: 'Bob', last_name: 'Archer'))
    end

    it 'be a collection of visitors including the associated users' do
